...
public class UserService {
    ...
    public UserDTO setUserLockStatus(Long id, Map<String, Object> params) {
        try {
            if (id == null) {
            ...
            user.setLocked(params.get("locked") == null ? user.getLocked() : Boolean.valueOf(params.get("locked").toString()));

            User userAfterUpdate = userRepository.save(user);
            return userMapper.userToUserDTO(userAfterUpdate);
        } catch (Exception e) {
            throw new MyException(ExceptionCode.SERVICE, e.getMessage());
        }
    }
